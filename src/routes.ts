export const APP_BASE_URL = "http://localhost:3000";

export const LOGIN_PAGE_SUFFIX = "/login";
export const LOGIN_PAGE = APP_BASE_URL + LOGIN_PAGE_SUFFIX;

export const SIGNUP_PAGE_SUFFIX = "/signup";
export const SIGNUP_PAGE = APP_BASE_URL + SIGNUP_PAGE_SUFFIX;

export const CREATE_PERSON_PAGE_SUFFIX = "/createPerson";
export const CREATE_PERSON_PAGE = APP_BASE_URL + CREATE_PERSON_PAGE_SUFFIX;

export const ADD_EXPENSE_PAGE_SUFFIX = "/addExpense";
export const ADD_EXPENSE_PAGE = APP_BASE_URL + ADD_EXPENSE_PAGE_SUFFIX;

export const GROUPS_PAGE_SUFFIX = "/groups";
export const GROUPS_PAGE = APP_BASE_URL + GROUPS_PAGE_SUFFIX;

export const STATISTICS_PAGE = "/statistics"
