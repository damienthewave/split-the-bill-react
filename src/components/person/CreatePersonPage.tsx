...
import { emptyPersonReadDto, PersonCreateDto, PersonReadDto } from "../../api/person/personDtos";
...
const CreatePersonPage: React.FC<CreatePersonPageProps> = ({
  ...
  useEffect(() => {
    getPerson()
      .catch(() => {});
    loadCurrencies()
      .catch((error: ApiCallError) => setCurrencyApiErrorMsg(error.message));
  }, [getPerson, loadCurrencies]);

  const [currencyApiErrorMsg, setCurrencyApiErrorMsg] = useState<string>("");

  if (person !== emptyPersonReadDto)
    return <Redirect to="/" />;


  const createPersonFormValidation = yup.object().shape({
    name: yup.string().required("Required!"),
    currency: yup.string().required("Required!").oneOf(currencies.map(currency => currency.abbreviation), "Required!")
  });
  ...
  return <div className="cpp-wrapper">
              ...
              >
                ...
                <Form.Group>
                  ...
                  >
                    ...
                    {currencies.map(currency => <option key={currency.id}>{currency.abbreviation}</option>)}
                  </Form.Control>
                  <Form.Control.Feedback type={"invalid"}>
                    {errors.currency}
                  </Form.Control.Feedback>
